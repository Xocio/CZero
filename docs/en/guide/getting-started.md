...
1. Get the latest module zip from the [download page](/en/download).
2. Flash the zip in Magisk / KernelSU / APatch — flashing automatically installs the companion **CZeroX** app, used to view status and adjust the configuration.
3. Follow the volume-key prompts during flashing:
   ...
   > When you inherit, the black/white lists and custom path lists carry over as-is, and `config.json` is **merged** with the new defaults — values you tuned are kept, and newly added options are filled in.

4. Reboot.

> If **CZeroX** didn't install automatically (e.g. a signature mismatch), grab it separately from the [download page](/en/download).
...
Just flash the new zip over the old install. Choose **inherit config** during flashing and your black/white lists, custom paths, app rules and suppression list are all kept — as are the values you already tuned in `config.json`, with newly added options filled in automatically. Normally there is nothing to re-adjust after an update.

Only if the old `config.json` is already corrupt and cannot be merged does it fall back to the new defaults, and the installer says so in its output.
